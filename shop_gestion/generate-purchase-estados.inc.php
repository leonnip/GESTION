...
<body>
	<?php
		...
		try {
			...
			$orderT->Order_Transaction($db, $cupon, $_direcc, $ipremota);
			...
			if($db->Commit()) {
				...
				$_SESSION['__Orden__'] = $datos->IdOrden;
				$server = $_SERVER['HTTP_HOST'];
				$key = "|elpaisseleccion_";
				$key_tar_ok = 'tar_ok'; $key_tar_ko = 'tar_ko';
				$key_paypal_ok = "paypal_ok"; $key_palpal_ko = "paypal_ko";
				
				/*RETURN URL OK - KO VISA MASTERCARD*/
				$URL_TAR_OK = "http://".$server."/transaction-successfully.php?visa=".sha1($key_tar_ok.$key)."&ok_ko=".base64_encode('tar_ok');
				$URL_TAR_KO = "http://".$server."/transaction-successfully.php?visa=".sha1($key_tar_ko.$key)."&ok_ko=".base64_encode('tar_ko');
				
				/*RETURN ULS OK - KO PAYPAL*/
				$URL_PAYPAL_OK = "http://".$server."/transaction-successfully.php?paypal=".sha1($key_paypal_ok.$key)."&ok_ko=".base64_encode('paypal_ok');
				$URL_PAYPAL_KO = "http://".$server."/transaction-successfully.php?paypal=".sha1($key_palpal_ko.$key)."&ok_ko=".base64_encode('paypal_ko');
				
				$Comercio = 'ELPAISSELECCION';
				...
				$MerchantCode = 322992710;
				$MerchantURL = 'http://'.$server.'/visa-paypal/checkout-process-visa.inc.php';
				$MerchantName = 'elpaisseleccion.com';
