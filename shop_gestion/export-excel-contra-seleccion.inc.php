<?php
    ...
    header("Content-Disposition: attachment; filename=pedidos_contra_elpaisseleccion_select".$date.".xls");
	...
	if (isset($_POST['estados'])) {
		...
		$thead = '<table border="1" bordercolor="#CCCCCC" cellpadding="10" cellspacing="10">
		   		<tr height="30px">
					...
					<td bgcolor="yellow" valign="middle" style="text-align:center; width:100px;font-weight: bold; font-size:13px;">TOTAL FACTURAR</td>
					<td bgcolor="red" valign="middle" style="text-align:center; width:100px;font-weight: bold; font-size:13px;">TOTAL A COBRAR</td>
					...
					<td bgcolor="yellow" valign="middle" style="text-align:center; width:300px;">NUMERO DE ORDEN</td>
					<td bgcolor="yellow" valign="middle" style="text-align:center; width:300px;">REMITENTE</td>
				</tr>
		    ';
		...
		while ($i < $n) {			
			...
			//$SQL .= "AND ((TRIM(lineasorden.EstadoPedido) = 'Transito' AND TRIM(lineasorden.EstadoPedido) = 'Transito') OR (TRIM(lineasorden.EstadoPedido) = 'Enviado' AND TRIM(lineasorden.EstadoPedido) = 'Transito'))";
			$db->setQuery($SQL);
			$row = $db->execute();
			...
			if (mysqli_num_rows($row) > 0) {
				...
				echo '
					<tr>
						...
						<td style="font-size:12px;text-align:center; font-weight: bold;">'.(($result1->Cantidad * $result1->Precio) + ($result1->GEnvios)).'&euro;</td>
						<td style="font-size:12px;text-align:center; font-weight: bold;">'.(($result1->Cantidad * $result1->Precio) + ($result1->GEnvios)).'&euro;</td>
						...
						<td style="font-size:12px;text-align:center">'.$result1->IdOrden.$_SESSION['TPV_ORDEN'].'</td>
						<td style="font-size:12px;text-align:center">'.$_SESSION['NAME_TIENDA'].'</td>
					</tr>
				';
			 }
			$i++;
