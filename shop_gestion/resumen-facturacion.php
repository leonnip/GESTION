                                	...
                                	<fieldset id="group_1">
			                        	<table id="resultSearch_Opt" cellpadding="1" cellspacing="1">
											...
											if (isset($_POST['consultar'])) {									
												...
												$SQL .= "WHERE TRIM(ordenes.EstadoPago) = 'ok' AND TRIM(lineasorden.EstadoPedido) <> 'Anulado' AND ordenes.FechaOrden >= '$fecha1' AND ordenes.FechaOrden <= '$fecha2' ";
												...
												for($i = 0; $i <= $totalOfertas - 1; $i++ ) {	*/							
													...
													$SQL .= "opcionesoferta.Iva, SUM(lineasorden.Cantidad) AS Unidades,SUM(lineasorden.Subtotal) as SubTotal, SUM(lineasorden.GastosEnvio) AS GEnvio, ordenes.Tramitado ";
													$SQL .= "FROM ordenes ";
													$SQL .= "INNER JOIN lineasorden ON ordenes.IdOrden = lineasorden.IdOrden ";
													$SQL .= "INNER JOIN opcionesoferta ON lineasorden.Talla = opcionesoferta.Id ";
													$SQL .= "INNER JOIN productos ON lineasorden.IdProducto = productos.IdOferta ";
