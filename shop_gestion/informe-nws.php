    ...
    <div id="content-body">
    	<div id="general" class="content">
        	<div id="Contenido" style="padding-top: 20px; overflow: auto; overflow-x:hidden"> 
            	...
            	<ul class="tabs">
    		    	<li class="active"><a href="#tab1"><span>Informe NewsLetter</span></a></li>
                ...
                <div class="tab_container">                	                    
                    ...
                    <div id="tab1" class="tab_content">
                        ...
                        <div id="resultSearch">
                        	<h3><img src="images/result.png" style="float: left;" />&nbsp;Informe NewsLetters <?php echo $nombre; ?></h3><br />
                            ...
                            <div id="dvData0">
	                        	<table id="resultSearch_Opt" cellpadding="1" cellspacing="1">
    	                        	...
    	                        	<thead>
                                    	<th width="50px">IdNw</th>
        	                        	<th width="100px">Fecha Envio</th>                                    
            	                        <th width="250px">Asunto</th>
                                        <th width="150px">Archivo</th>
                	                    <th width="100px">Total Enviadas</th>
                    	            	<th width="100px">Total Leidas</th>
                            	    </thead>                                
                                	<?php
									if (isset($_POST['consultar'])) {									
										...
										//for($i=$fechaInicio; $i<=$fechaFin; $i+=86400){	
											...
											$SQL = "SELECT nwdatos.*, nwtracking.*, count(nwtracking.IdPagina) as leidas FROM nwdatos 
													inner JOIN nwtracking ON nwdatos.IdNwd = nwtracking.IdPagina 
													WHERE nwdatos.nwdFecha >= '$fecha1' AND nwdatos.nwdFecha <= '$fecha2'
													group by nwtracking.IdPagina";
													
											//$SQL .= "WHERE nwdatos.Fecha >= '$fecha1' AND nwdatos.Fecha <= '$fecha2' ";
										
											$db->setQuery($SQL);
											$result = $db->loadObjectList();
											
											$fechass = explode('-', $result1->NwdFecha);
											$Y = $fechass[0];
											$M = $fechass[1];
											$D = $fechass[2];
											...
											foreach($result as $result1) {						
												echo '
													<tr height="24px">
														<td><label>'.$result1->IdNwd.'</label></td>												
														<td><label>'.$result1->NwdFecha.'</label></td>
														<td><label>'.utf8_encode($result1->NwdNombre).'</label></td>
														<td><label>'.$result1->tx_pagina.'</label></td>
														<td><label>'.$result1->NwdEnviadas.'</label></td>
														<td><label>'.$result1->leidas.'</label></td>
													</tr>
													';	
												$totalEnviadas = $totalEnviadas + $result1->NwdEnviadas;
												$totalLeidas = $totalLeidas + $result1->leidas;
											
												$ff = explode(' ', $fechaTexto);
												$fff = $ff[0]. ' '.$ff[1];
												
												$graph[] = array("fecha"=>$result1->NwdFecha, "enviadas"=>$result1->NwdEnviadas, "leidas"=>$result1->leidas);								
											}
										//}
										...
										echo '
											...
											<tfoot style="color: white; font-size: 14px; font-variant:small-caps; background: #1F88A7">
    	    	        	                	<tr height="30px" style="color: white; font-size: 14px; font-variant:small-caps;">
													...
													<td bgcolor="#1F88A7"><span></span></td>                        	                
                        	    		            <td bgcolor="#1F88A7"><span>'.$totalEnviadas.'</span></td>                            	    		        
													<td bgcolor="#1F88A7"><span>'.number_format($totalLeidas).'</span></td>
	                                	    	</tr>
		                                	</tfoot>	
										';
										...
										<tbody id="dataCustomer">
											<tr><td colspan="5" height="24px"></td></tr><tr><td colspan="6" height="24px"></td></tr>
	                	        	        <tr><td colspan="6" height="24px"></td></tr><tr><td colspan="6" height="24px"></td></tr>
    	                	                <tr><td colspan="6" height="24px"></td></tr><tr><td colspan="6" height="24px"></td></tr>
        	                	       	    <tr><td colspan="6" height="24px"></td></tr><tr><td colspan="6" height="24px"></td></tr>
            	                	       	<tr><td colspan="6" height="24px"></td></tr><tr><td colspan="6" height="24px"></td></tr>                	                    
	                               		</tbody>
                            ...
                            <?php if(isset($_POST['consultar'])) { ?>
	                            <script type="text/javascript">
							        ...
							        function drawChart() {
										...
										['Fecha', 'Enviadas', 'Leidas'],											
										<?php for($i=0; $i < $totalGraph; $i++) {
											echo "['".$graph[$i]['fecha']."',".$graph[$i]['enviadas'].",".$graph[$i]['leidas']."],";
										 } ?>
							        ]);
	
							        var options = {
							          title: 'Rendimiento NewsLetter',
									  vAxis: {title: 'Fechas',  titleTextStyle: {color: 'red'}}
							        };
