    ...
    <div id="content-body">
    	<div id="general" class="content">
        	<div id="Contenido" style="padding-top: 20px; overflow: auto; overflow-x:hidden"> 
            	...
            	<ul class="tabs">
    		    	<li class="active"><a href="#tab1"><span>Informe Call-Center</span></a></li>
                ...
                <div class="tab_container">                	                    
                    ...
                    <div id="tab1" class="tab_content">
                        ...
                        <div id="resultSearch">
                        	<h3><img src="images/result.png" style="float: left;" />&nbsp;Informe Ordenes Call-Center <?php echo $nombre; ?></h3><br />
                            ...
                            <div id="dvData0">
	                        	<table id="resultSearch_Opt" cellpadding="1" cellspacing="1">
    	                        	...
    	                        	<thead>
                                        ...
                                        <th width="100px">Total Ordenes Oficina</th>
                	                    <th width="100px">Total Ordenes Call</th>
                    	            	<th width="100px">Precio Orden</th>
									...
									if (isset($_POST['consultar'])) {									
										...
										for($i=$fechaInicio; $i<=$fechaFin; $i+=86400){	
											...
											$SQL = "SELECT SUM(lineasorden.Cantidad) AS totalProductos FROM ordenes ";
											$SQL .= "INNER JOIN lineasorden ON ordenes.IdOrden = lineasorden.IdOrden ";
											$SQL .= "WHERE ordenes.FechaOrden >= '$fechaff' AND FechaOrden <= '$fechaff' AND ordenes.EstadoPago = 'ok' ";
											$SQL .= "AND TRIM(ordenes.Tramitado) <> 'usuario-web' AND TRIM(lineasorden.EstadoPedido) <> 'Anulado' ";
											$SQL .= "UNION ";
																					
											$SQL .= "SELECT COUNT(DISTINCT(ordenes.IdOrden)) AS totalOficina FROM ordenes ";
											$SQL .= "INNER JOIN lineasorden ON ordenes.IdOrden = lineasorden.IdOrden ";
											$SQL .= "WHERE ordenes.FechaOrden >= '$fechaff' AND FechaOrden <= '$fechaff' AND ordenes.EstadoPago = 'ok' AND TRIM(ordenes.Tramitado) <> 'usuario-web' AND TRIM(ordenes.Tramitado) <> 'call-center' ";
											$SQL .= "AND TRIM(lineasorden.EstadoPedido) <> 'Anulado' ";
											$SQL .= "UNION ";
											
											$SQL .= "SELECT COUNT(DISTINCT(ordenes.IdOrden)) AS totalOrdenes FROM ordenes ";
											$SQL .= "INNER JOIN lineasorden ON ordenes.IdOrden = lineasorden.IdOrden ";
											$SQL .= "WHERE ordenes.FechaOrden >= '$fechaff' AND FechaOrden <= '$fechaff' AND ordenes.EstadoPago = 'ok' ";
											$SQL .= "AND TRIM(ordenes.Tramitado) <> 'usuario-web' AND TRIM(lineasorden.EstadoPedido) <> 'Anulado'";
											...
											foreach($result as $result1) {												
												if ($var == 1) { 
													...
													if(empty($result1->totalProductos))
														...
														$totalOficina = $result1->totalProductos;
												} else if($var == 3) {
											...
											echo '
												<tr height="24px">
													<td><label>'.$fechaTexto.'</label></td>												
													<td><label>'.$totalProductos.'</label></td>
													<td><label>'.$totalOficina.'</label></td>
													<td><label>'.$totalOrdenes.'</label></td>
													<td><label>'.$precioCallcenter.'&euro;</label></td>
										...
										echo '
											...
											<tfoot style="color: white; font-size: 14px; font-variant:small-caps; background: #1F88A7">
    	    	        	                	<tr height="30px" style="color: white; font-size: 14px; font-variant:small-caps;">
													...
													<td bgcolor="#1F88A7"><span>'.$totalOF.'</span></td>  
