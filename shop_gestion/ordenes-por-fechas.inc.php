<?php
	function ordenesFechas($web) {
		...
		$SQL = "SELECT ordenes.IdOrden, lineasorden.Id 
				FROM ordenes INNER JOIN lineasorden ON ordenes.IdOrden = lineasorden.IdOrden 
				...
				WHERE lineasorden.EstadoPedido != 'Anulado' AND ordenes.EstadoPago ='ok' AND (ordenes.FormaPago = 'contra-rembolso' OR ordenes.FormaPago = 'tarjeta' OR ordenes.FormaPago = 'paypal') 
		...
		//FIN
				...
				$SQL = "SELECT *, 
						ordenes.IdOrden, 
						lineasorden.Id as lineasOrden, lineasorden.Cantidad, lineasorden.GastosEnvio, opcionesoferta.Opcion, opcionesoferta.Precio, ordenes.Total, ordenes.Hora, direcciones.*,	usuarios.Dni, productos.Nombre_Producto
						...
						INNER JOIN direcciones ON relordendireccion.IdDireccion = direcciones.IdDireccion AND usuarios.Id = direcciones.D_IdCliente 
						LEFT JOIN imagenes ON productos.IdOferta = imagenes.IdOferta AND imagenes.Estado = 1 
						WHERE (ordenes.FormaPago = 'contra-rembolso' OR ordenes.FormaPago = 'tarjeta' OR ordenes.FormaPago = 'paypal') 
						AND ordenes.EstadoPago = 'ok' AND lineasorden.EstadoPedido != 'Anulado' 
						AND ordenes.FechaOrden >= '$fecha1' AND ordenes.FechaOrden <= '$fecha2' 
						$addSQL ";
				$db->setQuery($SQL);
				$row = $db->execute();
